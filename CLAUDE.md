...
- **Parser** (`parser/`): Recursive descent parser producing an AST. Grammar: `list → pipeline ((; | && | || | &) pipeline)*`, `pipeline → command (| command)*`, `command → compound_cmd | simple_cmd`, `compound_cmd → if_cmd | while_cmd`, `if_cmd → 'if' list 'then' list ('elif' list 'then' list)* ('else' list)? 'fi'`, `while_cmd → 'while' list 'do' list 'done'`. AST nodes: `List` (sequence with operators), `Pipeline` (pipe-connected commands), `Command` interface (implemented by `SimpleCmd`, `IfCmd`, `WhileCmd`), `SimpleCmd` (assignments + args as `[]lexer.Word` + redirections), `IfCmd` (clauses with condition/body lists + optional else), `WhileCmd` (condition + body). `CloneList` deep-copies AST subtrees so loops can re-expand on each iteration. Reserved words (`then`, `elif`, `else`, `fi`, `do`, `done`) terminate list parsing via stop words. Recognizes `NAME=VALUE` assignments before command words. `&` marks the preceding pipeline for background execution.
