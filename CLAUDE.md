...
- **Expander** (`expander/`): Five-phase expansion on the AST: (1) tilde expansion (`~` → `$HOME`, `~user` → user's home dir, only unquoted); (2a) arithmetic expansion (`$((expr))`) via recursive descent evaluator (`expander/arith.go`) — supports `+`, `-`, `*`, `/`, `%`, comparisons, logical/bitwise operators, ternary `?:`, parenthesized grouping, and variable references (bare `x` or `$x`); (2b) command substitution (`$(cmd)` and `` `cmd` ``) via a `SubstFunc` callback that recursively lex→parse→expand→executes, capturing stdout through a pipe — `CmdSubst` results are Unquoted (subject to globs), `CmdSubstDQ` results are DoubleQuoted (no globs); (3) variable expansion (`$VAR`, `${VAR}`, `$?`, `$$`, `$#`, `$@`, `$*`, `$0`-`$9`) respecting quoting — no expansion in SingleQuoted parts; (4) glob expansion (`*`, `?`, `[...]`) on unquoted args only, using `filepath.Glob`. Builds glob patterns that escape metacharacters in quoted parts.
