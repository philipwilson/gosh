...
- Functions: `fname() { body; }` defines a function stored in `shellState.funcs`. Called like commands with positional parameters (`$1`-`$9`, `$#`, `$@`/`$*`). `return [N]` exits the function with status N. Supports recursion — positional params are saved/restored per call. Function names are included in tab completion.
...
- `shellState` holds variables, export set, function table, positional parameters, last exit status, editor, terminal info, and job table
