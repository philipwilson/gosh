...
- **Parser** (`parser/`): Recursive descent parser producing an AST. Grammar: `list → pipeline ((; | && | || | &) pipeline)*`, `pipeline → command (| command)*`, `command → compound_cmd | simple_cmd`, `compound_cmd → if_cmd`, `if_cmd → 'if' list 'then' list ('elif' list 'then' list)* ('else' list)? 'fi'`. AST nodes: `List` (sequence with operators), `Pipeline` (pipe-connected commands), `Command` interface (implemented by `SimpleCmd` and `IfCmd`), `SimpleCmd` (assignments + args as `[]lexer.Word` + redirections), `IfCmd` (clauses with condition/body lists + optional else). Reserved words (`then`, `elif`, `else`, `fi`) terminate list parsing via stop words. Recognizes `NAME=VALUE` assignments before command words. `&` marks the preceding pipeline for background execution.
